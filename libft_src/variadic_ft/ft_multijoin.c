...
/*   Updated: 2025/04/10 16:21:36 by chrlomba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	calculate_total_length(t_join *join)
{
	...
	total_size = ft_strlen(join->blue)
		+ ft_strlen("[") + ft_strlen(join->nc);
	total_size += ft_strlen(join->green)
		+ ft_strlen(join->user) + ft_strlen(join->nc);
	total_size += ft_strlen("@")
		+ ft_strlen(join->cyan) + ft_strlen(join->host_name);
	total_size += ft_strlen(join->nc)
		+ ft_strlen(join->blue) + ft_strlen("]") + ft_strlen(join->nc);
	total_size += ft_strlen(" - ")
		+ ft_strlen(join->blue) + ft_strlen("[") + ft_strlen(join->nc);
	total_size += ft_strlen(join->bold)
		+ ft_strlen(join->pwd);
	total_size += ft_strlen(join->nc)
		+ ft_strlen(join->blue) + ft_strlen("]") + ft_strlen(join->nc);
	total_size += ft_strlen("\n")
	...
	total_size += ft_strlen(join->nc)
		+ ft_strlen(join->pink) + ft_strlen(" $ ") + ft_strlen(join->nc) + 1;
	return (total_size);
}
...
static void	copy_to_result_part1(char *result, t_join *join)
{
	...
	ptr += ft_strlen(ft_memcpy(ptr,
				join->host_name, ft_strlen(join->host_name)));
	ptr += ft_strlen(ft_memcpy(ptr, join->nc, ft_strlen(join->nc)));
	ptr += ft_strlen(ft_memcpy(ptr, join->blue, ft_strlen(join->blue)));
	ptr += ft_strlen(ft_memcpy(ptr, "]", ft_strlen("]")));
	ptr += ft_strlen(ft_memcpy(ptr, join->nc, ft_strlen(join->nc)));
...
char	*ft_multijoin(t_join *join)
{
	...
	copy_to_result_part1(result, join);
